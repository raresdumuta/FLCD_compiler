...
public class MyScanner {
    ...
    private final List<String> operators = Arrays.asList("+", "-", "*", "/", "%", "=", ":", "<", "ge", "se", ">", "ne", "and", "or", "!");
    ...
    private List<Pair> PIF = new ArrayList<>();
    ...
    public List<String> TokenizeLine(String line) {
        ...
        while (i < line.length()) {
            ...
            if (line.charAt(i) == '"') {
                ...
                // separators without space, tab and new line
    ...
    public void scanLine(int line, List<String> tokenizedLine) {
        ...
        for (int i = 0; i < tokenizedLine.size(); ++i) {
            ...
            if (isConstant(token)) {
                ...
                System.out.println("Token " + token + " on position: " + position + " constant" + "\n");
            } else if ((token.equals("-") || token.equals("+")) && (isNumber(tokenizedLine.get(i + 1))) &&
                    (lastToken.equals(":") || lastToken.equals("("))) { // number case
                ...
                var position = symbolTable.search(token);
                PIF.add(new Pair(code, position));
                System.out.println("Token " + token + " on position: " + position + " identifier");
            } else {
                System.out.println("Error at line " + line + ". Invalid token: " + token);
            }
            lastToken = token;
